...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (existingVerificationToken) {
      ...
      return res.status(400).json({ status: 0, message: 'Mail already sent for email verification and token is still valid' });
    }
  ...
  try {
    ...
    if (name) {
      ...
      if (user && user.role === 'child') {
        ...
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) {
          return res.status(401).json({ status: 0, message: 'Invalid name or password' });
        }

        // Generate JWT token for child user (no OTP)
        const token = jwt.sign({ userId: user.userId, role: user.role }, process.env.JWT_SECRET, { expiresIn: '15d' });
    ...
    if (!user || user.role !== 'child') {
      ...
      const isMatch = await bcrypt.compare(password, user.password);
      if (!isMatch) {
        return res.status(401).json({ status: 0, message: 'Invalid name/email or password' });
      }
...
app.post('/create-guardian',verifyParentRole,async (req, res) => {
  ...
  const parentFamily =[user.familyId];
